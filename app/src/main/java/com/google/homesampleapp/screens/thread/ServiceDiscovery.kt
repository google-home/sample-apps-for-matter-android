 ...
 *
 * The exception to the rule is the Thread Border Router, which will expose the Thread devices
 * in the Thread Network, which register using the Service Registration Protocol (SRP)

 * Once the service (device) is found, it is resolved by the
 * [ResolveListener], which will add the device information to the list of [resolvedDevices]. A
 * list is used here (and not a map) because it makes the iteration easier when user selects
 * a BR in the pop up dialog
 *
 ...
 * screens/commissionable/mdns. Matter has its own specific libraries that encapsulate
 * nsdManager and Matter-specific mDNS/SD code
 */
 ...
 * limitations of NsdManager on Android V and previous, we use a lock and a short delay to
 * prevent simultaneous resolving of services.
 */
class DiscoveryListener(
  ...
  override fun onServiceFound(service: NsdServiceInfo) {
    if (service.serviceType != serviceType) { // Service type is the string containing the protocol and transport layer for this service.
      ...
      viewModelScope.launch(Dispatchers.IO) {
        ...
        nsdManager.resolveService(
          service,
          ResolveListener(resolvedServices)
        )
 ...
 *
 * It will log part of the information found and add the data to [resolvedDevices] for future
 * usage when we want to show a list of known Border Routers in the local network.
 */
