...
class ThreadViewModel @Inject constructor() : ViewModel() {
  ...
  fun getGPSThreadPreferredCredentials(activity: FragmentActivity) {
    ...
    ThreadNetwork.getClient(activity).preferredCredentials.addOnSuccessListener { intentSenderResult ->
      intentSenderResult.intentSender?.let { intentSender ->
        ...
        "threadClient: no preferred credentials found", activity
      )
   ...
   *
   * The first credentials set become the preferred credentials. Thus, whenever installing a
   * new border router, always follow the procedure
   *
   * 1. Check if a set of preferred credentials exists in GPS (use [isPreferredCredentials] to
   *    a random set)
   * 2. If preferred credentials already exist, set those to your TBR, and update GPS credentials
   *    with that information
   * 3. If preferred credentials don't exist, create a set (E.g. use the random credentials
   *    shown below), set those to your TBR and update the GPS credentials with that information.
   *    Your set of credentials will thus become the preferred credentials
   */
  fun setGPSThreadCredentials(activity: FragmentActivity) {
    actionOnOTBRDialog(activity, Dispatchers.Main) { serviceInfo ->
      ...
      if (selectedThreadBorderRouterId == null) {
        ...
        enterTBRCredentialsDialog.setMessage("Use the preferred credentials (default), create random or copy your own Base16-encoded credentials")
        ...
        if (credentials != null) {
          base16Credentials = BaseEncoding.base16()
  ...
  private fun setGMSThreadRandomCredentials(borderRouterId: ByteArray, activity: FragmentActivity) {
    ...
    enterNetworkName.setPositiveButton("OK") { _, _ ->
      val credentials = ThreadNetworkCredentials.newRandomizedBuilder()
        .setNetworkName(enterNetworkNameEditText.text.toString()).build()
  ...
  private fun setGPSThreadCredentials(
    ...
    try {
      credentials = ThreadNetworkCredentials.fromActiveOperationalDataset(
    ...
    credentials?.let {
      ThreadNetwork.getClient(activity).addCredentials(threadBorderAgent, credentials)
        .addOnSuccessListener {
          ...
          ToastTimber.e("threadClient: Error adding the new credentials: " + getStatusCodeString((e as ApiException).statusCode), activity)
        }
    }
  }
  ...
  fun getOTBRActiveThreadCredentials(activity: FragmentActivity) {
    actionOnOTBRDialog(activity) { serviceInfo ->
      ...
      if (response.first.responseCode in OtbrHttpClient.okResponses) {
        val otbrDataset = ThreadNetworkCredentials.fromActiveOperationalDataset(
  ...
  fun showWorkingSetQRCode(activity: FragmentActivity) {
    ...
    try {
      ...
      if (credentials != null) {
        val qrCodeContent =
          threadCredentialsQRCodePrefix + BaseEncoding.base16()
            .encode(credentials.activeOperationalDataset)
  ...
  fun readQRCodeWorkingSet(activity: FragmentActivity) {
    ...
    scanner.startScan()
      .addOnSuccessListener { barcode ->
        try {
          val qrCodeDataset = ThreadNetworkCredentials.fromActiveOperationalDataset(
      ...
      .addOnCanceledListener {
        ToastTimber.d("QR Code scanning cancelled", activity)
      }
      .addOnFailureListener { e ->
        ToastTimber.e("Error reading QR Code: $e", activity)
      }
  }

  /**
   * Creates a dialog where user may pick a Border Router from the ones found on the local network
   * via mDNS/Service Discovery. The caller must provide a lambda function that will be called
   * with the [NsdServiceInfo] of the selected border router
   */
  private fun actionOnOTBRDialog(
    ...
    selectTBRDialog.setTitle(
      "Select the Border Router. Ensure the BR has the REST API enabled"
    )
    ...
    val borderRouterStringList = borderRouterLocalList.map { serviceInfo ->
  ...
  /**
   * There is no API to clear the preferred credential's storage. This method displays a message
   * on how to perform it in development phones via adb
   */
  ...
  /**
   * Consumes the value in [_threadClientIntentSender] and sets it back to null. Needs to be
