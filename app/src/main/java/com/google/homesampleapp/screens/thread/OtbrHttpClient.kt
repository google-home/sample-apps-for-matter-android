...
/**
 * OtbrHttp object has some useful methods, enums and properties for getting/setting information
 * to OpenThread Border Routers using the HTTP REST API:
...
object OtbrHttpClient {
  ...
  /**
   * Suspend function that generically creates an HTTP request and gives back its response.
   * This was created mostly for interacting with the OTBR HTTP, but it is generic enough for
   * general use.
   *
   * When using a GET for a credential, you may either use [acceptMimeType] == "text/plain",
   * which will return the credentials in hex/base16 TLV format, or [acceptMimeType] ==
   * "application/json", which will return the credentials in JSON format. See the OpenThread
   * HTTP Rest API for more information (link above)
   */
  suspend fun createJsonHttpRequest(
    ...
    return withContext(Dispatchers.IO) {
      ...
      urlConnection.requestMethod = when (verb) {
        Verbs.GET -> "GET"
        Verbs.POST -> "POST"
        Verbs.DELETE -> "DELETE"
        Verbs.PUT -> "PUT"
