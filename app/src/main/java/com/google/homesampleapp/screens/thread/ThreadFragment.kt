...
class ThreadFragment : Fragment() {
  ...
  private fun setupUiElements() {
    ...
    binding.setGPSButton.setOnClickListener {
      viewModel.setGPSThreadCredentials(requireActivity())
    }
    ...
    binding.readQRCode.setOnClickListener {
      viewModel.readQRCodeWorkingSet(requireActivity())
    }

    binding.showQRCode.setOnClickListener {
      viewModel.showWorkingSetQRCode(requireActivity())
    }
  ...
  private fun updateThreadInfo(credentials: ThreadNetworkCredentials?, title: String = "") {
    ...
    if (credentials != null) {
      textBox = "NetworkName: " + credentials.networkName +
        "\nChannel: " + credentials.channel +
        "\nPanID: " + credentials.panId +
        "\nExtendedPanID: " + BaseEncoding.base16()
        .encode(credentials.extendedPanId) +
        "\nNetworkKey: " + BaseEncoding.base16()
        .encode(credentials.networkKey) +
        "\nPskc:" + BaseEncoding.base16().encode(credentials.pskc) +
        "\nMesh Local Prefix: " + BaseEncoding.base16()
        .encode(credentials.meshLocalPrefix)

      tlv = BaseEncoding.base16()
